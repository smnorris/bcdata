...
def test_bc2pg():
    runner = CliRunner()
    result = runner.invoke(
        cli, ["bc2pg", AIRPORTS_TABLE, "--db_url", DB_URL]
    )
    assert result.exit_code == 0
...
def test_bc2pg_table():
    ...
    assert result.exit_code == 0 and "whse_imagery_and_base_maps.testtable" in DB_CONNECTION.tables
...
def test_bc2pg_schema():
    ...
    assert result.exit_code == 0 and "testschema.gsr_airports_svw" in DB_CONNECTION.tables
...
def test_bc2pg_maxworkers():
    runner = CliRunner()
    result = runner.invoke(
        cli, ["bc2pg", RIVERS_TABLE, "--db_url", DB_URL, "--pagesize", 5000, "--max_workers", 4]
    )
...
def test_bc2pg_append():
    runner = CliRunner()
    result = runner.invoke(
        cli, ["bc2pg", RIVERS_TABLE, "--db_url", DB_URL, "--query", "WATERSHED_GROUP_CODE='COWN'"]
    )
    result = runner.invoke(
        cli, ["bc2pg", RIVERS_TABLE, "--db_url", DB_URL, "--append", "--query", "WATERSHED_GROUP_CODE='VICT'"]
    )
    assert result.exit_code == 0 and RIVERS_TABLE in DB_CONNECTION.tables
...
def test_bc2pg_append_fid_small():
    runner = CliRunner()
    result = runner.invoke(
        cli, ["bc2pg", RIVERS_TABLE, "--db_url", DB_URL, "--fid", "waterbody_poly_id", "--query", "WATERSHED_GROUP_CODE='COWN'"]
    )
    result = runner.invoke(
        cli, ["bc2pg", RIVERS_TABLE, "--db_url", DB_URL, "--fid", "waterbody_poly_id", "--append", "--query", "WATERSHED_GROUP_CODE='VICT'"]
    )
    assert result.exit_code == 0 and RIVERS_TABLE in DB_CONNECTION.tables
    query = "select waterbody_poly_id from whse_basemapping.fwa_rivers_poly where watershed_group_code = 'VICT'"
    assert [r[0] for r in DB_CONNECTION.query(query)] == [710022126,710022158]
...
def test_bc2pg_append_fid_large():
    runner = CliRunner()
    result = runner.invoke(
        cli, ["bc2pg", RIVERS_TABLE, "--db_url", DB_URL, "--fid", "waterbody_poly_id", "--query", "WATERSHED_GROUP_CODE<>'COWN'"]
    )
    result = runner.invoke(
        cli, ["bc2pg", RIVERS_TABLE, "--db_url", DB_URL, "--fid", "waterbody_poly_id", "--append", "--query", "WATERSHED_GROUP_CODE='COWN'"]
    )
    assert result.exit_code == 0 and RIVERS_TABLE in DB_CONNECTION.tables
    query = "select waterbody_poly_id from whse_basemapping.fwa_rivers_poly where watershed_group_code = 'VICT'"
    assert [r[0] for r in DB_CONNECTION.query(query)] == [710022126,710022158]
